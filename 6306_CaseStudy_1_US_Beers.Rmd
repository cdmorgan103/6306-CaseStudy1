...
Beer ID:   | Unique identfier of the beer.
...
Field Name | Field Description
-----------|--------------------
Brew ID:   | Unique identfier of the brewery.
...
ggplot(data=med.st.abv, aes(x=med.st.abv$State,y=med.st.abv$ABV)) + geom_bar(stat="identity") + coord_flip()
ggplot(data=med.st.ibu, aes(x=med.st.ibu$State,y=med.st.ibu$IBU)) + geom_bar(stat="identity") + coord_flip()
...
ggplot(data=m1,aes(x=m1$ABV*100,y=m1$IBU)) + geom_point() + geom_smooth(method="lm") + xlab("Alcohol Content") + ylab("Bitternes") + ggtitle("Beer Bitternes vs Alcohol Content")
