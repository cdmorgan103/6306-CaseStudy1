...
With the merged data, we can begin a quick pass at removing duplicates in our dataset. We do this by looking for data that has white spaces or is stamped with a year or unique marking within parenthesis. We do not claim that this will capture all posiblities for duplicate data, but it greatly reduces duplicates for the purpose of our analysis. 
Duplicate data must share the same beername,brewery name, abv, ibu, and style to be removed. The final numbers reveal 2,410 unique crafted beers by all the microbreweries in the US. The two lists below represents the first 6 observations and last 6 observation respectivaly for the combined dataset.
...
One of the biggest challanges is empty of missing data in the fields. By utilizing our joined data, we simply sum NA's and print for each column. This provides the number of empty values per column or field in our dataset. This allows to asses how good is the data, and account for any problem that may cause.
...
```
The "IBU" field has 966 empty values, this is 40% of our dataset. It is our recomendation to research for these values and try to fill it in before making any desision on it. In contrast we have the "ABV" with 62 empty values, which represent 2.5% of our dataset, this number should not skew our data analysis for ABV.

##4. Compute the median alcohol content and international bitterness unit for each state. Plot a bar chart to compare.
Median alcohol content is computed using our joined dataset and then printed using a ggplot funciton. The results show that the median Alcohol By Volume ranges from 4% to 6.25% from state to state.  The 2.5% missing values on ABV are not enough to affect the outcome of these median values.

However, for the IBU median calculation may not be accurate due to the 40% missing values in the dataset. Even though the median can be calculated, this calcuation may differ enough from the original population. The state's median ranges for IBU are from 0 to 61. States like South Dakota with 0 is one of the states that is missing all the IBU data. There may be states that have enough missing IBU values to change their median outcome.  
...
Next we calculate the max ABV and IBU and print the results.
Colorado has the brewery that produces the higest Alcohol By Volume beer. Even though the 2.5% missing data may take Colorado's claim of having the highest ABV beer, the probability is low. However, we cannot say the same for the IBU first place. Oregon,the state with the highest IBU beer title, may not be safe. With 40% missing data, there may be other states out there with beers with an IBU larger than the one in Oregon. 
```{r MaxABV}
...
Refrenecing our joined dataset, we can print summary statistics for ABV. 
From the summary statistics can be infered that it is probable that the ABV variable follows a normal distribution becuase the median and the mean are very close. There is a slight rigth skew to the data, but nothing that will indicate an extreme departure from normality. One thing to notice is that, 75 % of all the beers have a 5% ABV or more. However, the intequartely distance is only 1.8 %. Meaing that 50 % of the data is between 5 and 6.8% of ABV.
``` {r, ABVSummary}
...
There is visual evidence that there is a positive linear relation between the alcohol content and the IBU values (R-Squared 0.44), meaning that 44% of the IBU variation is explained by the Alcohol By Volume variable. Therefore, other factors may affect the IBU as well and they are not acounted in this regression.
For every one percent increase in ABV value, there is an increase of 12.2 units in the IBU average (pvalue < 0.001).

There is a caveat on the regresion analysis that is the 40 % missing data in IBU. This could change the regresion coeficients since currently there is no way to know where that 40% falls in IBU. Take this regression analysis with this in mind. 
